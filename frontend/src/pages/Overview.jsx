...
function Overview() {
  ...
  const getStatusBadge = (chapterStatus) => {
    switch (chapterStatus) {
      case 'completed':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-900/20 rounded-full">
      ...
      case 'building':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/20 rounded-full">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-full">
            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
  ...
  if (error || !explanation) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          <p className="text-red-600 dark:text-red-400 mb-4">{error || 'Explanation not found'}</p>
          <button
            onClick={() => navigate('/')}
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      ...
      {/* Chapters List */}
      <div>
        <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Chapters</h2>
            ...
            explanation.chapters.map((chapter, index) => {
              ...
              return (
                <CardWrapper
                  key={chapter.id}
                  {...cardProps}
                  className={`block p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg transition-all ${
                    isClickable
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {isClickable && (
                      <svg className="w-5 h-5 text-gray-400 dark:text-gray-500 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
